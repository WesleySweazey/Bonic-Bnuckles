...
void Mesh::CreateBox(vec2 size, vec2 offset)
{
    ...
    {
        VertexFormat( vec2( -size.x/2, -size.y/2 ) + offset, MyColor( 255, 255, 255, 255 ),vec2(0.0f, 1.0f), vec2(0.0f, 0.0f)),
        VertexFormat(vec2(-size.x / 2,  size.y / 2) + offset, MyColor(255, 255, 255, 255), vec2(0.0f, 1.0f), vec2(0.0f, 1.0f)),
        VertexFormat(vec2(size.x / 2,  size.y / 2) + offset, MyColor(255, 255, 255, 255),  vec2(1.0f, 1.0f), vec2(1.0f, 1.0f)),
        VertexFormat(vec2(size.x / 2, -size.y / 2) + offset, MyColor(255, 255, 255, 255),  vec2(1.0f, 0.0f), vec2( 1.0f, 0.0f ) ),
    };
...
void Mesh::CreatePlane(vec2 worldSize, ivec2 vertexCount, vec2 pivotPoint, vec2 uvRepeat)
{
    ...
    for( int y = 0; y < vertexCount.y; y++ )
    {
        for( int x = 0; x < vertexCount.x; x++ )
        {
            ...
            verts[y * vertexCount.x + x].m_Normal.Set(0.0f, 1.0f, 0.0f);
        }
    }
