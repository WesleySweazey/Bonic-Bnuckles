...
void MyMemory_ValidateAllocations(bool breakIfThereAreLeaks)
{
    ...
    {
        MemObject* obj = static_cast<MemObject*>( pNode );
...
void* operator new(size_t size, const char* file, unsigned long line)
{
    ...
    MemObject* mo = static_cast<MemObject*>( malloc( size + sizeof(MemObject) ) );
    mo->m_file = file;
    mo->m_line = line;
    ...
    return reinterpret_cast<char*>( mo ) + sizeof(MemObject);
}
...
void operator delete(void* ptr, const char* file, unsigned long line)
{
    ...
    MemObject* mo = reinterpret_cast<MemObject*>( ( static_cast<char*>( ptr ) - sizeof(MemObject) ) );
